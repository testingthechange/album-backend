...
const PUBLISHED_PREFIX = (process.env.PUBLISHED_PREFIX || "public/players").replace(
  /^\/+|\/+$/g,
  ""
);

// IMPORTANT: include your actual static-site origin(s)
const ALLOWED_ORIGINS = [
  "https://blackout-web.onrender.com",
  "https://smartbridge-xr2d.onrender.com",
  "http://localhost:5173",
  "http://127.0.0.1:5173",
].filter(Boolean);
...
app.use(
  cors({
    origin: (origin, cb) => {
      ...
      // keep permissive to avoid blocking in production
      return cb(null, true);
    },
...
/**
 * ✅ FIX FOR YOUR CURRENT FRONTEND BUILD
 * Your deployed JS is calling:
 *   POST /upload-to-s3?projectId=...
 *
 * This route accepts multipart/form-data with:
...
app.post("/upload-to-s3", upload.single("file"), async (req, res) => {
  try {
    ...
    if (!file?.buffer) return res.status(400).json({ ok: false, error: "missing file" });
    ...
    await s3.send(
      new PutObjectCommand({
        ...
        Metadata: {
          projectid: projectId,
          originalfilename: safeStr(file.originalname),
          kind: "audio",
        },
      })
    );
...
app.get("/api/publish/:shareId/manifest", async (req, res) => {
  try {
    ...
    try {
      ...
      return res
        .status(404)
        .json({ ok: false, error: `manifest not found at s3://${bucket}/${manifestKey}` });
    }

    const tracksArr = Array.isArray(m?.tracks)
      ? m.tracks
      : Array.isArray(m?.album?.tracks)
      ? m.album.tracks
      : null;
